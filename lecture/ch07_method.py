...
#  $ 입력 매개변수의 순서가 매우 중요
#  $ 최초에는 함수 호출문을 동작하지만, 함수 종료 후에는 변수로서 활용 됨
#  $ 반환값이 있는 경우 반환값을 담는 변수, 반환값이 없는 경우 None
print(sum_two_valus(5, 10))
...
# 7. 변수의 범위
#  + 변수가 참고 가느안 코드상의 범위를 명시
#  + 함수 내의 변수는 자신이 속한 코드 블록이 종료되면 소멸
#  + 이렇게 특정 코드블록에서 선언된 변수를 지역변수
#  + 가자 상단에 정의되어 프로그램 종료 전까지 유지되는 변수를 전역 변수
