...
#  ex) 계산기(덧셈, 뺄셈, 나눗셈, 곱셈)
#  각각 4개를 만드는 것이 재사용성 UP

# 3. 함수를 만들어야 하는 경우
#  + 공통적으로 사용되는 코드는 함수로 생성
#  + 복잡한 로직이 사용되는 경우 식별 가능한 이름의 함수 
...
def sum_two_value(x, y):
    ...
    return n
# 3. 함수 호출
#  + 함수 호출 방법은 함수 정의시 만들어진 함수 이름을 사용해서 호출
#  + 함수 호출시 입력 매개변수와 함수 정의시 입력 매개변수는 동일한 개수여야 함
#  $ 함수 호출 후 함수가 종료된 경우 호출했던 곳으로 넘어감.
#  $ 입력 매개변수의 순서가 매우 중요
#  $ 최초에는 함수 호출문으로 동작하지만, 함수 종료 후에는 변수로서 활용됨
...
#  + return 다음에 오는 코드는 실행 안 됨(return을 만나면 함수 종료)
...
#   + 같은 이름의 지역 변수와 전역변수가 존재하는 경우 가까운 곳이 우선순위 높음
...
#   + 전달되는 parameter의 갯수가 고정적이지 않은 경우
#   가. *args     -> Tuple
#   나. **kwargs  -> Dict
...
# 9. Type hint
#   + 함수의 parameter와 반환값의 자료형 타입을 명시
#   + 코드 실행과는 무관
